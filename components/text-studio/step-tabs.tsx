...
export function StepTabs({ activeTab, onTabChange }: StepTabsProps) {
  ...
  return (
    <div
      className="w-full overflow-x-auto pb-2 md:pb-0 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0"
      role="tablist"
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 border",
                isActive
            ...
            >
              <Icon className={cn("w-4 h-4", isActive ? "text-primary" : "text-muted-foreground")} aria-hidden="true" />
              {tab.label}
            </button>
          )
        })}
