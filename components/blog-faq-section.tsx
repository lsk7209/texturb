  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 id="blog-faq-heading" className="text-2xl font-bold text-slate-900">
          이 글에 대해 자주 묻는 질문
        </h2>
      </div>
      ...
      <div className="space-y-3">
        {faqData.items.map((item, index) => (
          <div key={index} className="border border-slate-200 rounded-lg overflow-hidden">
            ...
            {openIndex === index && (
              <div id={`blog-faq-answer-${index}`} className="p-4 pt-0 text-slate-600 leading-relaxed" role="region">
                {item.answer}
              </div>
            )}
          </div>
