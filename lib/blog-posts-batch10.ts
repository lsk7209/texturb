...
**사례 3: 상품 데이터 마이그레이션**
쇼핑몰 상품 데이터를 신규 플랫폼으로 이전할 때 상품명, 설명 컬럼에 전각 공백(U+3000), Non-Breaking Space(U+00A0) 등이 섞여 있는 경우가 많다. <code>=TRIM(CLEAN(SUBSTITUTE(A1, CHAR(160), " ")))</code> 조합 공식으로 전처리하면 마이그레이션 오류를 크게 줄일 수 있다.
...
**사례 2: 학술 논문 메타데이터 추출**
저널 논문 PDF 수백 편에서 제목, 저자, 초록을 자동으로 추출할 때, pdfplumber의 <code>extract_words()</code> 메서드로 텍스트 위치 정보를 함께 추출하면 각 섹션을 좌표 기반으로 구분할 수 있다. OCR이 필요한 스캔 논문은 Tesseract lang='kor+eng'로 처리한다.

**사례 3: 스캔 영수증 처리**
경비 정산용 스캔 영수증에서 금액, 날짜, 가맹점명을 추출할 때 Tesseract DPI 300 설정이 인식률을 크게 높인다. 추출 후 정규식으로 날짜 패턴(<code>\d{4}[-./]\d{2}[-./]\d{2}</code>)과 금액 패턴(<code>\d{1,3}(,\d{3})*원</code>)을 매칭하면 구조화된 데이터로 변환할 수 있다.
