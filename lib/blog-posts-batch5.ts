...
## 한글·영문 혼용 제목 픽셀 계산 예시

혼용 제목의 실제 픽셀 너비를 추산하면 잘림 여부를 미리 판단할 수 있다.

| 제목 예시 | 픽셀 추산 | 결과 |
|---|---|---|
| \`SEO 최적화 가이드 - 구글 검색 상위 노출법\` | 한글 17자×23px + 영문 3자×8px = 415px | 안전 |
| \`텍스터브 글자수 세기 도구 - 공백 포함·제외 계산 | Texturb\` | 한글 22자×23px + 영문 7자×8px = 562px | 안전(경계) |
| \`무료 온라인 글자수·단어수·문장수·문단수·줄수 계산기 총정리 완벽 가이드\` | 한글 30자×23px = 690px | 잘림 |

## 작성 체크리스트

- [ ] 제목의 한글 글자수가 22자 이내인가? (모바일 기준 20자 이내)
- [ ] 영문·한글 혼용 시 픽셀 환산 합계가 600px 이하인가?
- [ ] 메타 설명이 한글 70-80자(920px) 이내인가?
- [ ] 제목 앞부분에 핵심 키워드가 배치되어 있는가?
- [ ] title 태그와 H1이 동일하거나 유사한가?
- [ ] 키워드 스터핑(키워드 반복 나열)이 없는가?
- [ ] 구글 Search Console에서 재작성 여부를 확인했는가?

## 자주 묻는 질문

**Q. 제목을 짧게 쓰면 키워드가 부족해 SEO에 불리하지 않나요?**

A. 구글 공식 문서(Google Search Central)에 따르면 title의 역할은 페이지 주제를 정확하게 전달하는 것입니다. 핵심 키워드 1-2개를 자연스럽게 포함하되, 잘려서 표시되는 제목은 오히려 CTR(클릭률)을 낮춥니다. 검색어와 일치하는 키워드가 앞에 올수록 더 굵은 글씨로 강조 표시되어 주목도가 높아집니다. 구글은 사용자가 검색한 쿼리와 관련성이 높은 단어를 SERP 제목에서 굵게 표시하는데, 이 강조 효과를 활용하려면 핵심 키워드를 title 앞 1/3 이내에 배치하는 것이 실무 권장 전략입니다.

**Q. 구글이 title을 자동 재작성하면 어떻게 해야 하나요?**

A. 구글이 재작성하는 주된 이유는 title이 페이지 실제 내용과 다르거나, 너무 길거나, 브랜드명이 반복되는 경우입니다. Search Console의 "검색 결과" 보고서에서 실제 표시 제목과 원본 title을 비교하고, 내용 일치도를 높이는 방향으로 수정하면 재작성 빈도가 줄어듭니다. H1 태그와 title의 내용을 80% 이상 일치시키는 것이 권장됩니다.
...
\`opendict.korean.go.kr\`에서 "확인하다"를 검색하면 점검하다, 검토하다, 살피다, 파악하다, 들여다보다 같은 유의어가 제시된다. 문맥에 맞는 단어를 골라 교체한다.
...
## 실전 적용 가이드 - 보고서 반복 단어 제거

실제 공문서 초고에서 발견되는 전형적인 반복 패턴과 개선 예시를 살펴본다.

**수정 전 (반복 과다)**
> "이번 프로젝트의 목표를 달성하기 위해 해당 방안을 검토하였습니다. 검토 결과, 방안 A가 가장 적합한 방안으로 판단됩니다. 방안 A를 통해 목표를 달성할 수 있을 것으로 예상됩니다."

여기서 "방안"이 4회, "목표"가 2회, "달성"이 2회 반복된다.

**수정 후 (동의어 분산)**
> "이번 프로젝트의 성과 지표를 충족하기 위해 세 가지 대안을 검토하였습니다. 분석 결과, 방안 A가 가장 적합한 것으로 판단됩니다. 이 접근법을 채택하면 목표를 효과적으로 달성할 수 있을 것으로 예상됩니다."

"방안" 대신 "대안"과 "접근법"을 교체해 반복을 2회로 줄였다. 국립국어원 공문서 작성법 권장 기준(한 문단 내 동일 단어 2회 이하)을 충족한다.

## 자주 묻는 질문

**Q. 전문 용어는 반복이 불가피한데 어떻게 처리하나요?**

A. 기술 문서나 법령에서 특정 전문 용어(예: "계약서", "법인", "이의신청")는 문맥상 동의어 교체가 불가합니다. 이 경우 해당 용어를 정의 후 약칭이나 대명사로 지칭하는 방식을 사용합니다. 예를 들어 "이 약정서(이하 '약정서')"처럼 처음 한 번 정의하면 이후 "약정서"를 반복해도 독자가 혼란 없이 이해합니다. 국립국어원 공문서 기준도 전문 용어의 불가피한 반복은 허용합니다.

**Q. 자동화 도구로 반복 단어를 한 번에 교체할 수 있나요?**

A. 가능하지만 주의가 필요합니다. Python의 \`re\` 모듈이나 Word 매크로로 특정 단어를 일괄 교체할 수 있으나, 문맥 파악 없이 기계적으로 교체하면 의미가 달라질 수 있습니다. 예를 들어 "확인하다"를 전부 "점검하다"로 바꾸면 "신분을 확인하다"가 "신분을 점검하다"처럼 어색해집니다. 도구는 빈도 분석까지만 활용하고, 실제 교체는 문장 단위로 수동으로 진행하는 것이 안전합니다.
...
## 실전 예제 - Executive Summary 압축

임원 보고용 Executive Summary 전후 비교를 통해 분량 조정 감각을 익힌다.

**압축 전 (약 350자, 과다)**
> "2023년 하반기부터 고객 불만 접수가 증가하고 있는 상황에서, 이에 대한 근본적인 원인을 파악하고 개선 방안을 마련하기 위해 본 기획서를 작성하게 되었습니다. 다양한 내외부 자료를 검토한 결과, 고객 서비스 응답 속도가 주된 불만 요인으로 나타났으며, 이를 해결하기 위한 방안을 세 가지 측면에서 검토하였습니다."

**압축 후 (약 120자, 적정)**
> "고객 불만 증가(2023 하반기 +38%)의 주원인은 서비스 응답 지연입니다. 3가지 개선 방안(인력 증원/AI챗봇/SLA 재설정)을 검토했으며, 권장안은 AI챗봇 도입입니다."

핵심 수치를 포함하고 배경 설명을 삭제해 230자를 절감했다. 임원은 이 버전만으로 의사결정이 가능하다.

## 자주 묻는 질문

**Q. 기획서 분량 기준이 조직마다 달라서 어떻게 맞춰야 할지 모르겠습니다.**

A. 명시적인 기준이 없다면 보고 대상자의 선호를 파악하는 것이 우선입니다. 이전에 채택된 기획서를 참고하거나 보고 전 담당자에게 선호 분량을 확인하는 것이 좋습니다. 일반적인 안전 기준은 "임원에게는 A4 3매 이내, 팀 내 검토용은 10매 이내"입니다. 공공기관의 경우 행정안전부 공문서 작성 규정에 분량 기준이 명시된 경우도 있으니 관련 지침을 먼저 확인하세요.

**Q. 글자수를 맞추다 보면 중요한 내용이 빠지는 것 같아서 걱정됩니다.**

A. 중요한 내용은 별첨(Appendix)으로 분리하면 됩니다. 임원 보고용 본문에는 의사결정에 필요한 최소 정보만 담고, 근거 데이터·상세 분석은 "별첨 A", "별첨 B"로 구분해 뒤에 붙입니다. 임원이 필요 시 직접 참조할 수 있고, 보고 시간도 단축됩니다. 이 구조는 맥킨지·BCG 등 글로벌 컨설팅 기업의 표준 보고서 형식이기도 합니다.
...
## 실전 예제 - 직군별 소개 before/after

각 직군의 실제 개선 사례를 통해 작성 감각을 익혀본다.

**UI/UX 디자이너 (개선 전, 480자 - 과다)**
> "저는 5년간 모바일 앱 UI/UX 디자인 분야에서 다양한 프로젝트를 진행해왔으며, 사용자 중심 설계 방법론을 중시하는 디자이너입니다. 다양한 업계의 클라이언트와 협업하여 사용자 경험을 개선한 경험이 있으며, Figma와 Sketch를 주로 사용합니다."

**개선 후 (240자 - 적정)**
> "모바일 앱 UX 디자이너 5년 경력. 사용자 리서치 기반 설계로 앱 이탈률 23% 개선 경험. Figma 전문, Behance 포트폴리오 링크 참조."

**백엔드 개발자 (개선 전, 310자 - 과다)**
> "저는 Java와 Spring Boot를 활용한 백엔드 개발을 주로 담당하고 있으며, RESTful API 설계와 데이터베이스 최적화에 관심이 많은 개발자입니다. 최근에는 클라우드 환경에서의 마이크로서비스 아키텍처에 대해서도 공부하고 있습니다."

**개선 후 (200자 - 적정)**
> "Java/Spring Boot 백엔드 4년. RESTful API 설계·DB 쿼리 최적화 전문. AWS ECS 마이크로서비스 전환 경험(응답속도 40% 단축). GitHub 링크 첨부."

## 자주 묻는 질문

**Q. 포트폴리오 플랫폼(Behance, GitHub, 개인 사이트)마다 소개 글자수 기준이 다른가요?**

A. 네, 플랫폼마다 표시 방식이 다릅니다. Behance는 프로필 소개가 긴 텍스트도 표시되지만, 채용 담당자가 보는 화면에서는 처음 2-3줄만 노출됩니다. GitHub의 About 섹션은 160자 이내를 권장합니다. 링크드인 Summary는 최대 2,000자를 허용하지만 모바일에서는 처음 300자만 표시됩니다. 가장 중요한 정보(직군·핵심 성과·연락처)를 항상 앞 200자 안에 배치하는 것이 모든 플랫폼에 통용되는 원칙입니다.

**Q. 포트폴리오 소개에 연봉 협상에 유리한 내용을 어떻게 담아야 하나요?**

A. 직접적인 연봉 언급보다 수치화된 성과가 효과적입니다. "매출 30% 성장에 기여", "사용자 만족도 4.2→4.7 향상" 같은 구체적 수치는 인터뷰 단계에서 협상 레버리지가 됩니다. 스타트업보다 대기업 지원 시에는 규모감(팀 규모, 예산 규모)도 포함하면 좋습니다. 단, 수치 작성 시 NDA(비밀유지협약)로 공개가 제한된 내부 지표는 사용하지 않도록 주의해야 합니다.
...
LF(\ )는 Unix/Linux/Mac의 줄바꿈 방식이고, CRLF(\\r\ )는 Windows의 방식입니다. 혼용 시 코드 diff가 오염되거나 스크립트 실행이 실패할 수 있습니다. VS Code에서는 우측 하단 'CRLF/LF' 버튼으로, Git에서는 core.autocrlf 설정으로 통일합니다.
